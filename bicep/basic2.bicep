...
@description('NSG for the back-end subnet. THis is just an example. Tailor to your security requirements!')
resource nsgMidTierSubnet 'Microsoft.Network/networkSecurityGroups@2023-04-01' = {
  name: 'nsg-${location}-${appname}-${environment}-mt-${suffix}'
  location: location
  properties: {
    securityRules: [
      {
        name: 'AllowAll443InFromFE'
        properties: {
          protocol: 'Tcp'
          sourcePortRange: '*'
...
resource nsgMidTierSubnet_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = {
  scope: nsgMidTierSubnet
  name: 'to-la'
  properties: {
    workspaceId: laworkspace.id
    logs: [
...
resource vnet 'Microsoft.Network/virtualNetworks@2023-04-01' = {
  ...
  properties: {
    ...
    subnets: [
      ...
      {
        name: 'snet-${location}-${appname}-${environment}-mt-${suffix}'
        properties: {
          addressPrefix: '10.0.2.0/24'
          networkSecurityGroup: { 
            id: nsgMidTierSubnet.id
          }
          privateEndpointNetworkPolicies: 'Disabled'
          privateLinkServiceNetworkPolicies: 'Disabled'
        }
